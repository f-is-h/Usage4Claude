...
### Added
- **Pace graph**: A second graph style that plots each limit against the elapsed share of its window, next to a dotted diagonal marking even consumption, so being ahead of or behind pace reads at a glance. It works for both Claude and Codex, draws each point with its limit's row shape so similar colors stay distinguishable, plots the third and later weekly models, and leaves out Claude extra usage, which is a spend cap without a reset window. Labels follow the used/remaining toggle while points stay on used % to remain comparable with the diagonal, and placement keeps each label next to its dot when a stacked column would push it away. Weekly limits can pace against weekday time only. The pace math lives in the unit-tested `UsagePaceGraphMath`. Selected in Settings → Display → Graph Style; the types were renamed from linear/circular to pace/ring while keeping the stored raw values and the `linearGraphWeekdaysOnly` key, so existing choices carry over (#85)
- **Customizable notification thresholds**: The 5-hour limit gets one threshold slider, and weekly limits and extra usage get two-thumb sliders (50-100% in 5% steps), shared by Claude and Codex. Defaults keep the previous 90% and 75%/90%, and extra usage gains a 75% early reminder. `NotificationDecisionEngine` evaluates any number of thresholds per limit and sends one notification when several are crossed at once, decouples reset detection from user thresholds, silently marks thresholds already below current usage when settings change so they do not fire after relaunch or an account switch, re-arms extra usage thresholds once usage drops below them, keeps evaluating while notifications are off so re-enabling does not replay old crossings, names every per-model weekly limit by its real model, and migrates stored records to per-threshold keys
- **Menu bar icon follows the used/remaining toggle**: The flag moved from `UsageDetailView` state into `UserSettings`, reusing the existing `@AppStorage` key, and `UsageDisplayMode` became the single source for the conversion. The icon animates the flip frame by frame along the popover's spring (response 0.42, damping 0.78), hand-evaluated since AppKit has no equivalent; the arc keeps the overshoot and the number does not. Colors still key off the used figure, so a 90% used limit stays red while reading 10. The frame timer runs in `.common` mode against `systemUptime`, and the popover rings now use the same `UsageDisplayMode.Spring`
- **Fetch error indicator**: A transient failure shows a small orange indicator next to the provider title instead of a banner that grew the popover by 53pt. Clicking it opens a native popover with the error and when the displayed data was fetched. Closing a detail popover by clicking outside the main popover now closes both, and the event monitors are removed only after the popover has actually closed
- **Codex token diagnostics**: The diagnostic report shows the Codex access token's expiry and the outcome of refresh attempts (#88)

### Changed
- **Settings reorganized into five tabs**: Display, Data, Accounts, General and About replace the single General tab, with tab indices in a `SettingsTab` enum. Sections were renamed ("Menu Bar Appearance", "Limit Types"), the reset button reads "Reset All to Defaults", the menu entries became "Settings" and "Accounts", and the sponsorship links moved above the service status links with a pink GitHub Sponsor heart. The window no longer drops a cursor into the alias field or takes stray focus rings, and its size and position are pinned synchronously instead of fighting `setFrameAutosaveName` behind a 50ms delay
- **Codex credits shown as a balance**: Credits are a prepaid wallet with no limit, so the old percentage could only be 0 or 100. The hexagon now draws the balance itself (250, 2.5k, 25k; "<1" below one; ∞ for unlimited plans), solves its font size from the hexagon geometry, and uses two colors, healthy and exhausted. The percentage survives only as a binary signal for notifications
- **Codex reset announcement moved into the column header**: The badge sits next to the Codex title with a fixed row height, so it no longer pushes the graph around, and uses a megaphone icon instead of an arrow that read as a second refresh button. Clicking it opens a popover with the expected time, the quoted announcement and "Beta · Source: codex-reset.com", with the countdown updating every minute; the details used to live in a tooltip that never appeared inside the popover. French titles shortened to "Usage Claude" / "Usage Codex" to make room
- **Refresh timing**: Every request pushes the main timer back a full interval, so a popover open, manual refresh, ring tap or wake can no longer be followed seconds later by a scheduled fetch. Opening the popover always refreshes, including during a backoff, unless a request went out in the last 10 seconds (previously 30). Changing the refresh mode or interval no longer fetches immediately
- **Debug features gated behind the debug toggle**: Every debug control appears only while the master toggle is on, and simulated update, individual shape icons and keep-detail-window-open go through `effective*` accessors that also require it. In debug mode the Codex reset announcement comes solely from the debug scenario
- **Diagnostic step notes kept out of the UI**: They are developer-facing English and bypassed localization; they still go into the exported report
- **Documentation**: The README was rewritten in all 7 languages, with contributor avatar walls rendered by `scripts/render_contributors.py` from `docs/contributors.json`, which also covers translators and issue reporters. README images are now rendered offscreen from the real SwiftUI views by `scripts/render_docs_images.sh` through `DocsRenderMode` stand-ins for AppKit controls, and the screenshot skill was rewritten around it. The website was brought up to date with 3.4.1, and outdated guides moved to `docs/archive`

### Fixed
- **Bursts of requests triggering HTTP 429**: A smart monitoring mode change restarted the refresh timer, and `startRefreshing()` fetches immediately, so every mode change sent a second usage request within a few hundred milliseconds; Codex rounds were also counted twice, flipping the mode back and forth. Anthropic answered those bursts with 429 and `Retry-After: 0`, and the next tick retried at the normal interval, which kept the limit renewing for hours. Mode and interval changes now only reschedule the timer, each combined round is counted once, and automatic fetches back off per provider after 429, 5xx, network errors and Cloudflare blocks (1, 2, 4, 8, 16 then 30 minutes; 5 minutes for network errors; 1.0-1.2 jitter; positive `Retry-After` honored up to an hour). Manual refresh bypasses the backoff, a cancelled request no longer counts as a failure, and switching accounts clears that provider's backoff (#86, #90)
- **Codex asking to sign in again, especially with Advanced Security**: Codex OAuth access tokens are now reused until expiry and persisted in the Keychain across restarts, so launches no longer spend a refresh token. A 401 from the usage endpoint renews the access token and retries once before asking for a re-login, and credential failures that survive the retry trigger the re-login notification. A 429 or 5xx from the tier 1 SSR refresh backs off instead of escalating to the hidden WebView, which hit the same site and could flag healthy credentials as needing a re-login (#87, #88)
- **Codex data cleared by transient errors**: A rate limit, network error or 5xx replaced the Codex column with a full-screen error. Cached data now stays with the same indicator as Claude, and only credential errors clear it and offer "Go to Settings"
- **Menu bar numbers unreadable on a dark menu bar**: Percentages were hard-coded black, and nothing observed the menu bar's appearance, which follows the wallpaper rather than the system setting. Every colored renderer now takes its text and background colors from `button.effectiveAppearance`, observed via KVO with a 200ms debounce, and Color with Background uses a dark 0.30-alpha backdrop on a dark menu bar
- **Codex reset announcements missed**: The badge stayed hidden through the Watch raised before the Sep 12 reset, because 3.4.x read `/api/timeline`, the site's curated history. It now reads `latest_alert` from `/api/forecast` (about 6KB instead of 119KB), shows announcements without a stated time, keeps an overdue line past the stated time, and logs a warning when the site reports a signal the parser cannot read
- **Onboarding failed after a successful browser login**: The welcome screen fed the returned OAuth refresh token to `fetchOrganizations` as if it were a cookie, so sign-in ended with "failed to fetch organization ID". Browser login now finishes onboarding directly, Codex can be signed in during onboarding, Skip is offered on every page, a failed organization fetch keeps the window open, and finishing no longer opens the settings window
- **Onboarding and sign-in windows covering the browser**: They were `.floating` and drew above the browser they asked the user to switch to; they are ordinary windows now
- **Provider icons flickering in the popover header**: SwiftUI compares `Image(nsImage:)` by identity and the header built a new `NSImage` on every evaluation, so animated transactions crossfaded the icons. Icons are cached per size and backing scale
- **Ring toggle animation leaving stray marks**: Arcs shorter than 0.2% are skipped, so the spring settle no longer flashes a dot on an empty ring, and the outer sweep highlight that read as a phantom thin ring is removed

### Security
- **API responses no longer written to disk**: Sessions built on `URLSessionConfiguration.default` stored responses in the app's `Cache.db` despite `reloadIgnoringLocalCacheData`, which only skips reading the cache; a 3.4.1 install held cached claude.ai usage and chatgpt.com responses. All services now use `URLSessionConfiguration.uncached` (`.default` without a URL cache, since Codex token rotation needs the shared cookie storage), the reset announcement session is ephemeral, and `NetworkCachePolicy.applyAtLaunch` purges existing cached responses before zeroing the shared `URLCache` capacity
...
[3.5.0]: https://github.com/f-is-h/Usage4Claude/releases/tag/v3.5.0
