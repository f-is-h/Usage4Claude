...
final class ClaudeOAuthCoordinator: ObservableObject {
    ...
    func start(onAccountCreated: ((Account) -> Void)? = nil) {
        ...
        AppLog.event(.auth, "Claude OAuth opened the system browser and is waiting for authorisation on callback port \(port)")
    ...
    func submitManualCallback(_ pasted: String) -> Bool {
        ...
        guard query["code"] != nil || query["error"] != nil else {
            AppLog.error(.auth, "Claude OAuth: the pasted callback URL contains no authorisation code")
            return false
        }
        AppLog.event(.auth, "Claude OAuth is completing sign-in from the pasted callback URL")
    ...
    private func handleCallback(_ query: [String: String]) {
        ...
        guard let returnedState = query["state"], returnedState == pkce?.state else {
            AppLog.error(.auth, "Claude OAuth state validation failed; rejecting the callback")
            fail(L.WebLogin.codexOAuthFailed)
            return
        }
        if let error = query["error"] {
            AppLog.error(.auth, "Claude OAuth authorisation server returned an error: \(error)")
            fail(L.WebLogin.codexOAuthFailed)
            return
        }
    ...
    private func handleTokens(_ result: Result<ClaudeOAuthTokens, Error>) {
        ...
        case .failure(let error):
            AppLog.error(.auth, "Claude OAuth token exchange failed: \(error.localizedDescription)")
            fail(L.WebLogin.codexOAuthFailed)

        case .success(let tokens):
            guard !tokens.refreshToken.isEmpty else {
                AppLog.error(.auth, "Claude OAuth token response carried no refresh_token")
                fail(L.WebLogin.codexOAuthFailed)
                return
            }
    ...
    private func createAccount(tokens: ClaudeOAuthTokens, profile: Result<(email: String, orgId: String, orgName: String), Error>) {
        ...
        AppLog.event(.auth, "Claude OAuth sign-in succeeded; created account: \(account.displayName)")
        finishCleanup()
    }
