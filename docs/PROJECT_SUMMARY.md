...
**需求背景：**
- 用户希望在活跃使用时能更及时看到用量变化（1分钟刷新）
- 在未使用时希望减少 API 调用，避免触发限流（最长10分钟刷新）
- 需要一个自动的、智能的频率调整机制
...
2. **渐进降频规则**
   - 活跃模式：连续3次无变化 → 短期静默（3分钟后）
   - 短期静默：连续6次无变化 → 中期静默（18分钟后）
   - 中期静默：连续12次无变化 → 长期静默（60分钟后）
   - 长期静默：保持当前模式
...
1. **数据模型扩展** (UserSettings.swift)
   - `RefreshMode` 枚举：智能/固定
   - `RefreshInterval` 枚举：4个级别 (1/3/5/10分钟)
   - `MonitoringMode` 枚举：4个智能模式级别
   - 状态追踪属性：`lastUtilization`, `unchangedCount`, `currentMonitoringMode`

2. **智能逻辑集成** (MenuBarManager.swift)
   - `fetchUsage` 中调用智能模式更新
   - `restartTimer` 使用 `effectiveRefreshInterval` 支持智能/固定模式

3. **UI 交互** (SettingsView.swift)
   - 刷新模式选择器（单选按钮）
   - 固定频率下拉菜单（仅在固定模式时显示）
...
- ✅ **智能节能**：静默时最长10分钟刷新，显著减少 API 调用（高达10倍）
- ✅ **及时响应**：活跃时1分钟刷新，用户体验良好
- ✅ **渐进降频**：4级平滑过渡，避免突变
- ✅ **灵活配置**：用户可选择智能或固定模式
- ✅ **简单可靠**：核心逻辑清晰，易于维护
...
为智能频率功能添加了完整的4语言支持：
- 英文：Smart Frequency / Fixed Frequency
- 日文：スマート頻度 / 固定頻度
- 简体中文：智能频率 / 固定频率
- 繁体中文：智慧頻率 / 固定頻率
...
### 问题13：URLSession 网络超时问题

**现象：**
```
API Response: {"five_hour":{"utilization":39,...}}
...
- API 请求成功返回数据（200 OK）
- 但随后出现大量 "Operation timed out" 错误
- 错误来自 URLSession 网络层，不是 Claude API 的返回

**原因分析：**

1. **每次请求都创建新的 URLSession**
```swift
// ❌ 问题代码
func fetchUsage(completion: @escaping (Result<UsageData, Error>) -> Void) {
    ...
    // session 创建后没有被 invalidate，连接未正确关闭
}
```

2. **连接管理问题**
   - URLSession 创建后占用系统资源
   - 没有显式 invalidate，连接可能未正确关闭
   - 累积的连接导致超时错误
...
**1. 使用共享的 URLSession 实例**

修改 `ClaudeAPIService.swift`：
...
// 资源超时：整个资源下载的超时时间
configuration.timeoutIntervalForResource = 60 // 60秒
```
...
1. **URLSession 生命周期管理**
   - 避免频繁创建/销毁 URLSession
   - 对于重复请求，复用同一个 session 实例
   - 在应用生命周期内保持 session 有效

2. **超时时间设置**
   - `timeoutIntervalForRequest`：单个请求超时
   - `timeoutIntervalForResource`：整体资源下载超时
   - 根据网络状况和 API 响应时间合理设置

3. **缓存策略**
   - 对于需要实时数据的 API，禁用缓存
   - 使用 `.reloadIgnoringLocalCacheData` 或 `.reloadIgnoringLocalAndRemoteCacheData`

4. **Cookie 管理**
   - 设置 `httpCookieAcceptPolicy = .always`
   - 确保 Session Key 正确传递
...
1. **单例或共享实例**：对于重复的网络请求，使用共享的 URLSession
2. **合理的超时配置**：根据实际情况设置超时时间
3. **正确的缓存策略**：实时数据禁用缓存
4. **资源清理**：虽然 URLSession 会自动管理，但保持良好的生命周期意识
