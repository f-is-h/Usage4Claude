...
- **两份发布材料，各司其职**（详见下一节分工表）：
  ...
  - `RELEASE_NOTES.md` — 面向用户的发布说明，CI 提取当前版本段落后**同时**注入
    Sparkle 弹窗（`appcast.xml` 的 `<description>`）和 GitHub Release 正文。
- **触发条件**：commit message 含 `[release]`/`[RELEASE]`，push 到 `main`，且本次改动
  包含 `CHANGELOG.md` 或 `RELEASE_NOTES.md`（发版通常两者都改）。
- **Sparkle 弹窗内容来自 RELEASE_NOTES.md**（不是 CHANGELOG）。用户在应用内“检查更新”
  看到的就是 RELEASE_NOTES 当前版本段落。
- **RELEASE_NOTES.md 必须有当前版本段落**：否则 Sparkle/Release 正文会为空。CI 的
...
- 触发前自检：commit 含 `[release]`、本次改动含 `CHANGELOG.md`/`RELEASE_NOTES.md`、目标分支 `main`。
...
.github/scripts/generate_release_notes.sh \
  .github/RELEASE_TEMPLATE.md <版本> /tmp/rn_preview.md RELEASE_NOTES.md      # 预览 Release 正文
...
- 发版前 `CHANGELOG.md` 与 `RELEASE_NOTES.md` 都要有当前版本段落。
